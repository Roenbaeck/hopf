...
\cite[Props.~5.7 and~5.9]{BourguignonDeschampsSentenac1973}, where
separated Killing variations and a related field-valued class are
obstructed.  Here the coupled-kernel classification, a two-curl coefficient,
and the spherical spectral gap make the sign explicit for arbitrary sums in
the full quotient.
...
$s^2(\alpha^2+\beta^2)/4$; the two pure Christoffel vectors lie in
opposite factors, so their pairing vanishes.  The squared area is
$1+O(s^2)$, so it does not change the quadratic coefficient.  This proves
...
\]
Since $\Sph^2$ has no harmonic one-forms, $F=0$ exactly when $E=0$,
and similarly for $G$ and $D$.
...
\begin{equation}\label{eq:rotational-oblique-curvature}
 ...
 \qquad(0<|s|<1).
\end{equation}
Thus this canonical transverse coupled kernel ray leaves the
...
\begin{corollary}[The diagonal rotational repair fails quartically]
...
harmonic and an exact cross-degree frame average, and verifies the
signed-SVD obstruction for an arbitrary $(1,1)$ block.
